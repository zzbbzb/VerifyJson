...
document.getElementById("addMainBtn").addEventListener(
  "click",
  function() {
...
document.getElementById("editMainBtn").addEventListener(
  "click",
  function() {
    if (typeof infoJson != "undefined") {
      PressEditBtn("");
...
function ShowMainArrayJson() {
  ...
  for (var key in infoJson) {
    ...
    var delkeyStr = '"' + key + '"';
    html +=
      "<button class='delBtn' id='delBtn_" +
      keyStr +
      "' onclick='PressDelBtn(" +
      keyPathStr +
      "," +
    ...
    for (var key in infoJson[i]) {
      ...
      if (
        ...
        if (GetJsonType(infoJson[i][key]) == "Array") {
          ...
          html +=
            " <button class='nextJsonBtn' id='button_" +
            keyStr +
            "' onclick='" +
            funcName +
            "' >展开</button><br/>";
...
function InitAddJson(json, newJson) {
  for (var key in json) {
    ...
    else if(type == "boolean")
    {
      ...
      newJson[key] = [];
      let aType = GetJsonType(json[key][0]);
      if (aType == "string")
      {
        newJson[key][0] = "";
      }
...
function InitJson(json, path, lastType) {
  if (GetJsonType(json) != "string" && GetJsonType(json) != "number" && GetJsonType(json) != 'boolean') {
    if (GetJsonType(json) == "Object") {
      if (lastType == "Object") {
  ...
  for (var key in json) {
    if (lastType == "Object") {
      if (
        GetJsonType(json[key]) != "string" &&
        GetJsonType(json[key]) != "number" && 
        GetJsonType(json[key]) != "boolean"
      ) {
        var aPath = "";
        if (path == "") {
          aPath = key;
  ...
  if (type == "Array") {
    ...
    if (windowKey != "m") {
      ...
      if (GetJsonType(tmpAddJson) == "Array") {
        ...
        for (var key in addJ) {
          if (
            GetJsonType(addJ[key]) == "string" ||
            GetJsonType(addJ[key]) == "number" ||
            GetJsonType(addJ[key]) == "boolean"
          ) {
            var inputKeyStr =
              "input_" + PathConvertToKey(newKeyPath) + "_" + key;
            var input = document.getElementById(inputKeyStr);
            if (input.value == "") {
              WriteInfo("设置值的时候不能为空", true);
              return;
      ...
      for (var key in addJson) {
        if (
          GetJsonType(addJson[key]) == "string" ||
          GetJsonType(addJson[key]) == "number"
        ) {
...
function SaveInputValue(keyPath, newKeyPath, key, windowPath, mainFlag, value) {
  ...
  var windowKey = GetPathLastKey(windowPath);
  if (windowKey == "m") {
    let jsonType = GetJsonType(addJson[key]);
    if (jsonType == "string")
    {
        addJson[key] = value;
    }
    ...
    {
        addJson[key] = parseInt(value);
    }
    else if(jsonType = "boolean")
    {
        addJson[key] = Boolean(value);
    }
    TipShadeHidden();
    ShowAddObjectJson(keyPath, newKeyPath, windowPath, mainFlag);
  }
